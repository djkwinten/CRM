...
// ── Kleuren ──────────────────────────────────────────────────────────────────
const BLAUW = rgb(0, 0.478, 1)        // #007AFF
const DONKER = rgb(0.11, 0.11, 0.13)  // ~#1C1C21
...
function sectieHeader(
  page: PDFPage, boldFont: PDFFont, titel: string,
...
function veldLabel(
  page: PDFPage, font: PDFFont, label: string, x: number, y: number
) {
  page.drawText(label, { x, y, size: 7, font, color: GRIJS_RND })
}

function tekstVeld(
  form: PDFForm, page: PDFPage, naam: string,
  x: number, y: number, breedte: number, hoogte = VELD_H,
  multiline = false
) {
...
function checkVeld(
  form: PDFForm, page: PDFPage, naam: string,
  x: number, y: number, label: string, font: PDFFont
) {
...
function tijdVeld(
  form: PDFForm, page: PDFPage, font: PDFFont, _boldFont: PDFFont,
  emoji: string, label: string, naam: string,
  x: number, y: number
): number {
  page.drawText(emoji, { x, y: y - 8, size: 9, font })
  page.drawText(label, { x: x + 16, y: y - 8, size: 8.5, font, color: DONKER })
  // Tijdveld rechts uitlijnen
  const tijdX = x + 260
...
function nummerveld(
  ...
  emoji: string, label: string, naam: string,
  x: number, y: number, breedte = 340
): number {
  page.drawText(emoji, { x, y: y - 8, size: 9, font })
  page.drawText(label, { x: x + 16, y: y - 8, size: 8, font, color: GRIJS_RND })
...
export async function generateBlankFormPDF(isTrouw: boolean): Promise<Uint8Array> {
  ...
  // Back-up contact
  y -= 4
  page.drawText('📞  Back-up contact op de avond', { x: MARGE, y: y - 2, size: 7.5, font: bold, color: DONKER })
  page.drawText('(ceremoniemeester of contactpersoon, indien niet de organisator)', { x: MARGE + 130, y: y - 2, size: 7, font, color: GRIJS_RND })
  y -= 6
  ...
  if (isTrouw) {
    y = tijdVeld(form, page, font, bold, '💒', 'Ceremonie', uid('uur_ceremonie'), MARGE, y)
  }
  y = tijdVeld(form, page, font, bold, '🥂', 'Receptie — start', uid('uur_receptie'), MARGE, y)
  y = tijdVeld(form, page, font, bold, '🥂', 'Receptie — einde', uid('uur_receptie_einde'), MARGE, y)
  if (isTrouw) {
    y = tijdVeld(form, page, font, bold, '🥂', '2e Receptie — start (optioneel)', uid('uur_receptie2'), MARGE, y)
    y = tijdVeld(form, page, font, bold, '🥂', '2e Receptie — einde (optioneel)', uid('uur_receptie2_einde'), MARGE, y)
  }
  y = tijdVeld(form, page, font, bold, '🍽️', 'Diner', uid('uur_diner'), MARGE, y)
  y = tijdVeld(form, page, font, bold, '🎂', 'Dessert', uid('uur_dessert'), MARGE, y)
  y = tijdVeld(form, page, font, bold, '🕺', 'Dansfeest', uid('uur_dansfeest'), MARGE, y)
  y = tijdVeld(form, page, font, bold, '🌙', 'Midnight Snack', uid('uur_midnightsnack'), MARGE, y)
  y = tijdVeld(form, page, font, bold, '🔚', 'Einduur', uid('einduur'), MARGE, y)
  ...
  if (isTrouw) {
    ...
    for (let i = 0; i < maxRijen; i++) {
      if (links[i]) {
        y = nummerveld(form, page, font, links[i][0], links[i][1], uid(`intrede_${i}_l`), MARGE, y, COL_W - 4)
        // rechterkolom start terug op dezelfde y
      }
      if (rechts[i]) {
        nummerveld(form, page, font, rechts[i][0], rechts[i][1], uid(`intrede_${i}_r`), MARGE + COL_W + 10, y + 18, COL_W - 4)
      }
    }
    y -= 4
  }
  ...
  // ══════════════════════════════════════════
  // SECTIE — TOESTEMMING FOTO
  // ══════════════════════════════════════════
  y = sectieHeader(page, bold, `${sectieNr + 3}. Toestemming Foto / Video`, MARGE, y - 4, A4_W - MARGE * 2)
  y -= 8
  page.drawText(
    'Geeft u toestemming aan DJ Kwinten om foto\'s/video\'s van het feest te gebruiken voor promotionele doeleinden?',
    { x: MARGE, y: y - 2, size: 8, font, color: DONKER }
  )
  y -= 12
  ...
  // ── Footer ──
  const footerY = 28
  page.drawLine({ start: { x: MARGE, y: footerY + 12 }, end: { x: A4_W - MARGE, y: footerY + 12 }, thickness: 0.4, color: rgb(0.85, 0.85, 0.87) })
  page.drawText('DJ Kwinten · Den Tandt Kwinten · 0498/21 64 48 · DJKWINTEN@gmail.com · BTW BE 0726.773.488', {
    x: MARGE, y: footerY, size: 6.5, font, color: GRIJS_RND,
  })

  // Zelfde footer op pagina 1
  const p1 = doc.getPage(0)
  const fp1Y = 28
  p1.drawLine({ start: { x: MARGE, y: fp1Y + 12 }, end: { x: A4_W - MARGE, y: fp1Y + 12 }, thickness: 0.4, color: rgb(0.85, 0.85, 0.87) })
  p1.drawText('DJ Kwinten · Den Tandt Kwinten · 0498/21 64 48 · DJKWINTEN@gmail.com · BTW BE 0726.773.488', {
    x: MARGE, y: fp1Y, size: 6.5, font, color: GRIJS_RND,
  })
...
export async function downloadBlankForm(isTrouw: boolean) {
  const filename = `djkwinten-vragenlijst-${isTrouw ? 'trouw' : 'algemeen'}.pdf`

  // Open alvast een nieuw tabblad vóór de async operatie (vermijdt popup-blocker)
  const win = window.open('', '_blank')

  const bytes = await generateBlankFormPDF(isTrouw)
  ...
  try {
    ...
    if (win && !win.closed) {
      // Gebruik het reeds geopende tabblad om de PDF te tonen
      win.location.href = url
      setTimeout(() => URL.revokeObjectURL(url), 15000)
    } else {
      // Fallback: probeer toch via <a> download
